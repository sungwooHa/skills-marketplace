"""Humanize KR v2.0.0 quantitative metrics calculator.

Extends metrics.py with the post-editese 3-axis model (simplification /
normalisation / interference) and 8 translationese-type detection signals
drawn from the Korean machine-translation / post-editing literature
(Toral 2019; Baker 1993; Toury 1995; types T1~T8).
...
Layering:
- The 8 baseline functions (comma_inclusion_rate ... lexical_diversity) are
  imported *as-is* from the sibling metrics.py — signatures and return shapes
  preserved. They are never redefined here. This is what keeps the older
  baseline.json comparisons regression-safe.
- This module adds 14 NEW pure functions for post-editese + T1~T8 detection,
  scored against baseline_v2.json.

Both (metrics.py, baseline.json) and (metrics_v2.py, baseline_v2.json) must
ship together: metrics_v2 imports metrics, and compute_all_v2 delegates to
compute_all, which reads baseline.json. Dropping either pair breaks the other.
...
# ---------------------------------------------------------------------------
# Import the baseline metrics module (regression-safe — signatures untouched)
#
# PATH ANCHORING: metrics.py is a SIBLING of this file. Anchor on __file__ and
# nothing else. An earlier revision walked three levels up and re-appended
# ".claude/skills/humanize-korean/references", which only resolved while this
# file sat in its original build-staging directory; once it moved next to
# metrics.py that expression pointed at a non-existent ".claude/.claude/..."
# path and `import metrics` raised ModuleNotFoundError for every caller that
# did not happen to have the references dir already on sys.path.
# ---------------------------------------------------------------------------
...
def _default_baseline_v2_path() -> str:
    # Must match the filename actually shipped in this directory. An earlier
    # revision looked for "baseline_v2_diff.json", which never existed here;
    # _load_baseline_v2 then returned {} and every v2 z-score came back None
    # with an empty warnings list — a silent no-op that looked like a pass.
